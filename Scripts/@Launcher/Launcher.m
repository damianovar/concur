classdef Launcher < handle
	...
	methods % non-static
		...
		function tLauncher = Launcher()
			...
			tLauncher.atValidChoices(tLauncher.MAIN_MENU).SHOW_DEFAULT_PARAMETERS	= 7;
			tLauncher.atValidChoices(tLauncher.MAIN_MENU).CHANGE_PARAMETERS			= 8;
			tLauncher.atValidChoices(tLauncher.MAIN_MENU).EXIT						= 9;
			%
			% fix the parameters for each menu
			...
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).SELECT_TAXONOMY				= 3;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).PLOT_KCG						= 4;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).PLOT_CENTRALITY_INDEXES		= 5;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).PLOT_CONNECTIVITY_INDEXES		= 6;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).LIST_MOST_CENTRAL_PREREQS		= 7;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).LIST_MOST_CENTRAL_DEVED		= 8;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).LIST_MOST_CONNECTED_PREREQS	= 9;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).LIST_MOST_CONNECTED_DEVED		= 10;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).SET_NODE_FILTER				= 11;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).DOWNLOAD_EXAM					= 12;
			tLauncher.atValidChoices(tLauncher.KCM_ANALYSIS).EXIT							= 13;
		   	%
